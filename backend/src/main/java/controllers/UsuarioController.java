...
public class UsuarioController implements HttpHandler {
    ...
    public void handle(HttpExchange ex) throws IOException {
        try {
            ...
            if (method.equals("GET") && path.equals("/usuarios")) {
            ...
            } else if (method.equals("GET") && path.equals("/usuarios/existe")) {
                String query = ex.getRequestURI().getQuery();
                String email = "";
                if (query != null && query.startsWith("email=")) {
                    email = java.net.URLDecoder.decode(query.split("=", 2)[1], "UTF-8");
                }
                ...
                if (nombre.isBlank()) {
                    send(ex, 400, "{\"error\":\"El nombre es obligatorio\"}");
                    return;
                }
                if (!email.contains("@")) {
                    send(ex, 400, "{\"error\":\"Email inválido\"}");
                    return;
                }
                if (password.length() < 6) {
                    send(ex, 400, "{\"error\":\"La contraseña debe tener al menos 6 caracteres\"}");
                    return;
                }
                if (carreraId <= 0) {
                ...
                if (rol.isBlank()) {
                    send(ex, 400, "{\"error\":\"Debes seleccionar un rol\"}");
                    return;
                }
                ...
                if (email.isBlank() || password.isBlank()) {
                    send(ex, 400, "{\"error\":\"Email y contraseña son obligatorios\"}");
                    return;
                }

                Map<String, Object> usuario = service.login(email, password);
                if (usuario != null)
                    send(ex, 200, gson.toJson(usuario));
                else
                    send(ex, 401, "{\"error\":\"Credenciales inválidas\"}");
